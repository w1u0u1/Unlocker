    ...
    public class Win32Process
    {
        ...
        public static List<Win32API.SYSTEM_HANDLE_INFORMATION> GetHandles()
        {
            ...
            try
            {
                ...
                ipHandle = ipHandlePointer + Marshal.SizeOf(ipHandlePointer);

                for (long lIndex = 0; lIndex < lHandleCount; lIndex++)
                {
                    shHandle = Marshal.PtrToStructure<Win32API.SYSTEM_HANDLE_INFORMATION>(ipHandle);
                    ipHandle += Marshal.SizeOf(new Win32API.SYSTEM_HANDLE_INFORMATION());
